...
\begin{equation}
  ...
  \label{eq:validation-xc-derivative-chain}
\end{equation}
The resulting tensor in
Eq.~\eqref{eq:validation-xc-derivative-chain} is contracted with
quadrature weights and AO basis values, transformed to the
occupied--virtual orbital basis, and inserted into the TDA or Casida
response matrices. Matched PySCF
...
\begin{table}[t]
\centering
\caption{B3LYP/def2-SVP S$_1$ excitation energies \(\Omega_1\) and
oscillator strengths \(f_1\) from matched PySCF and GradTDDFT
calculations. Excitation energies are reported in eV, while oscillator
strengths are dimensionless.}
\label{tab:b3lyp-s1-excitation-oscillator}
\begin{tabular}{llcccc}
\toprule
\multirow{2}{*}{Molecule} & \multirow{2}{*}{Solver} &
\multicolumn{2}{c}{PySCF} & \multicolumn{2}{c}{GradTDDFT} \\
...
\begin{minipage}{0.485\linewidth}
  ...
  \includegraphics[width=0.88\linewidth]{figures/h2plus_ground_hfx_implicit_scf_dissociation_paper_style.pdf}
\end{minipage}
\hfill
\begin{minipage}{0.485\linewidth}
  \centering
  \textbf{(b) Fixed density}\\[-0.3em]
  \includegraphics[width=0.88\linewidth]{figures/h2plus_ground_hfx_fixed_density_dissociation_paper_style.pdf}
\end{minipage}
...
\begin{minipage}{0.485\linewidth}
  \centering
  \textbf{(a) HFX+PT2}\\[-0.3em]
  \includegraphics[width=0.88\linewidth]{figures/h2_pt2_s1_tda_e1_total_dissociation_paper_style.pdf}
\end{minipage}
\hfill
\begin{minipage}{0.485\linewidth}
  \centering
  \textbf{(b) HFX/no-PT2}\\[-0.3em]
  \includegraphics[width=0.88\linewidth]{figures/h2_nopt2_s1_tda_e1_total_dissociation_paper_style.pdf}
\end{minipage}
...
mean absolute error of 18.2 meV and a maximum absolute error of
55.6 meV, whereas the updated no-PT2 model gives corresponding
...
\begin{minipage}{0.485\linewidth}
  \centering
  \textbf{(a) HFX+PT2}\\[-0.3em]
  \includegraphics[width=0.88\linewidth]{figures/n2_pt2strict_s1_tda_e1_total_dissociation_paper_style.pdf}
\end{minipage}
\hfill
\begin{minipage}{0.485\linewidth}
  \centering
  \textbf{(b) HFX/no-PT2}\\[-0.3em]
  \includegraphics[width=0.88\linewidth]{figures/n2_nopt2_s1_tda_e1_total_dissociation_paper_style.pdf}
\end{minipage}
...
\begin{equation}
  ...
  \label{eq:dissociation-e1-total}
\end{equation}
Reference curves for Eq.~\eqref{eq:dissociation-e1-total} are
generated with a fixed basis, spin sector, and active space across
the scan so that errors reflect the learned functional and response
treatment rather than changing reference definitions.
