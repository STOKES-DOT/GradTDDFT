...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare PySCF TDA and JAX-backbone TDA absorption spectra on one QM9 molecule."
    )
    ...
    parser.add_argument("--outdir", default="outputs/qm9_pyscf_vs_jax_tda")
...
def _fetch_molecule(db_path: Path, db_id: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    try:
        ...
        conn.close()
    if row is None:
        raise KeyError(f"QM9 id {db_id} not found in {db_path}")
...
def _write_summary(
    ...
    *,
    ...
    with path.open("w", encoding="utf-8") as f:
        f.write("QM9 single-molecule PySCF TDA vs JAX TDA comparison\n")
...
def main() -> None:
    ...
    if not db_path.exists():
        raise FileNotFoundError(f"QM9 db not found: {db_path}")
    ...
    stem = f"qm9_id{args.db_id}_{formula}_{str(args.xc).lower()}_{str(args.basis).lower()}".replace("*", "star")
    ...
    ax.set_title(f"QM9 id={args.db_id} {formula} | {str(args.xc).upper()}/{str(args.basis).upper()}")
    ax.grid(alpha=0.25)
