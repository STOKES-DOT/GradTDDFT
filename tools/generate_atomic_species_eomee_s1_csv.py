...
from atomic_species_extrapolation_common import atom_spec_map


HARTREE_TO_EV = 27.211386245988

TRAINING_COMPATIBLE_FIELDS = [
    "system",
    "split",
    "basis",
    "cart",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Generate closed-shell atomic EOM-EE-CCSD singlet S1 references "
            "for S1-only atom species extrapolation."
        )
    )
    p.add_argument("--preset", choices=("closed_shell_s1",), default="closed_shell_s1")
    ...
    p.add_argument("--outcsv", default="outputs/atomic_species_s1_631g/atomic_s1_references.csv")
    p.add_argument("--overwrite", action=argparse.BooleanOptionalAction, default=False)
    return p.parse_args(argv)


def _run_rhf(mol: gto.Mole, *, scf_conv_tol: float, scf_max_cycle: int) -> scf.hf.RHF:
    attempts = (
        dict(init_guess="minao", damping=0.0, level_shift=0.0, max_cycle=scf_max_cycle, use_newton=False),
        dict(init_guess="atom", damping=0.25, level_shift=0.5, max_cycle=max(scf_max_cycle, 200), use_newton=False),
...
def _compute_reference_row(
    ...
    *,
    ...
    return {
        ...
        "atomic_number": int(spec.atomic_number),
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger = RunLogger(outcsv.parent / "generate_atomic_s1_references.log")
    specs = atom_spec_map(preset=str(args.preset))
    requested = list(specs) if args.systems is None else [str(name) for name in args.systems]
    existing = {} if bool(args.overwrite) else _load_existing_rows(outcsv)
    rows_by_name: dict[str, dict[str, Any]] = {
        name: dict(row) for name, row in existing.items() if name in specs
    }

    for name in requested:
        if name not in specs:
            raise KeyError(f"Unknown atom {name!r}. Available: {sorted(specs)}")
        ...
        _write_rows(outcsv, [rows_by_name[key] for key in requested if key in rows_by_name])
        logger.log(f"[ref] wrote {outcsv}")

    rows = [rows_by_name[key] for key in requested if key in rows_by_name]
    ...
    summary = {
        ...
        "loss_boundary": "S1-only excited-state training; ground energies are schema/reference fields only.",
    }
    (outcsv.parent / "summary.json").write_text(json.dumps(summary, indent=2, sort_keys=True), encoding="utf-8")
    logger.log(f"[ref] complete count={len(rows)}")
