...
def _has_s1_supervision(args: argparse.Namespace) -> bool:
    return any(
        float(weight) > 0.0
        for weight in (args.s1_total_mse_weight, args.s1_total_mae_weight)
    )
...
def _objective_solver_name(args: argparse.Namespace) -> str:
    return str(args.excited_state_solver)
...
def _objective_display_label(args: argparse.Namespace) -> str:
    ...
    if kind == "e0_total":
        return "E0 total"
    solver_label = str(args.excited_state_solver).upper()
    if kind == "s1_total":
        return f"S1 total {solver_label}"
    return f"E0 + S1 total {solver_label}"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help=(
            ...
            "curve supplies target_s1_total_h."
        ),
    )
    ...
    p.add_argument(
        ...
        type=int,
        default=10,
        help="Write a train log line every N optimizer steps; use 1 for every epoch.",
    )
    p.add_argument(
        "--training-mode",
    ...
    p.add_argument(
        ...
        type=float,
        default=1.0,
        help="MSE weight for E1_total = E0 + Omega1.",
    )
    p.add_argument(
        "--s1-total-mae-weight",
        type=float,
        default=1.0,
        help="MAE weight for E1_total = E0 + Omega1.",
    )
    p.add_argument(
...
def _append_training_progress_row(
    path: Path,
    *,
    step: int,
    loss: float,
    s1_total_mae: float,
    scf_converged_fraction: float,
    scf_cycles_mean: float,
    scf_cycles_max: float,
    scf_final_rms_mean: float,
    scf_final_rms_max: float,
    learning_rate: float,
) -> None:
    _append_rows_csv(
        path,
        [
            {
                "step": int(step),
                "loss_pre_update": float(loss),
                "e1_total_mae_pre_update": float(s1_total_mae),
                "scf_converged_fraction": float(scf_converged_fraction),
                "scf_cycles_mean": float(scf_cycles_mean),
                "scf_cycles_max": float(scf_cycles_max),
                "scf_final_rms_mean": float(scf_final_rms_mean),
                "scf_final_rms_max": float(scf_final_rms_max),
...
def _s1_total_supervision_penalty(metrics: dict[str, Any]) -> float:
    return _metric_mean(metrics, "s1_total_loss", 0.0)


def _s1_total_supervision_mse(metrics: dict[str, Any]) -> float:
    return _metric_mean(metrics, "s1_total_mse", 0.0)


def _s1_total_supervision_mae(metrics: dict[str, Any]) -> float:
    return _metric_mean(metrics, "s1_total_mae", 0.0)


def _s1_total_predicted_metric(metrics: dict[str, Any]) -> float:
    return _metric_mean(metrics, "s1_total_predicted_h", float("nan"))


def _s1_total_target_metric(metrics: dict[str, Any]) -> float:
    return _metric_mean(metrics, "s1_total_target_h", float("nan"))
...
def _training_checkpoint_metadata(
    ...
    *,
    ...
    resolved_eval_use_tda = (
        (args.excited_state_solver == "tda") if eval_use_tda is None else bool(eval_use_tda)
    )
    return {
...
def _streaming_average_eval(
    ...
    s1_maes: list[float] = []
    s1_mses: list[float] = []
    scf_converged: list[float] = []
    scf_cycles_mean: list[float] = []
    scf_cycles_max: list[float] = []
    scf_final_rms_mean: list[float] = []
    scf_final_rms_max: list[float] = []
    for datum in data:
        ...
        scf_converged.append(_metric_scalar(metrics, "scf_converged_fraction"))
        scf_cycles_mean.append(_metric_scalar(metrics, "scf_cycles_mean"))
        scf_cycles_max.append(_metric_scalar(metrics, "scf_cycles_max"))
        scf_final_rms_mean.append(_metric_scalar(metrics, "scf_final_rms_mean"))
        scf_final_rms_max.append(_metric_scalar(metrics, "scf_final_rms_max"))
    return float(np.mean(losses)), {
        "s1_penalty": float(np.mean(s1_penalties)),
        "s1_mae": float(np.mean(s1_maes)),
        "s1_mse": float(np.mean(s1_mses)),
        "scf_converged_fraction": float(np.mean(scf_converged)),
        "scf_cycles_mean": float(np.mean(scf_cycles_mean)),
        "scf_cycles_max": float(np.max(scf_cycles_max)),
        "scf_final_rms_mean": float(np.mean(scf_final_rms_mean)),
        "scf_final_rms_max": float(np.max(scf_final_rms_max)),
    }


def _self_consistent_prediction_config(args: argparse.Namespace) -> MolecularTrainingConfig:
...
def _train_functional_streaming(
    ...
    training_data: tuple[MolecularTrainingDatum, ...],
    *,
    args: argparse.Namespace,
    logger: RunLogger,
    functional: Any,
    gs_training: MolecularTrainingConfig,
    ...
    if bool(args.skip_initial_eval):
        ...
        initial_metrics = {
            ...
            "s1_mae": 0.0,
            "s1_mse": 0.0,
            "scf_converged_fraction": float("nan"),
            "scf_cycles_mean": float("nan"),
            "scf_cycles_max": float("nan"),
            "scf_final_rms_mean": float("nan"),
            "scf_final_rms_max": float("nan"),
        }
        min_loss = float("inf")
    ...
    scf_converged_fraction_history = [initial_metrics["scf_converged_fraction"]]
    scf_cycles_mean_history = [initial_metrics["scf_cycles_mean"]]
    scf_cycles_max_history = [initial_metrics["scf_cycles_max"]]
    scf_final_rms_mean_history = [initial_metrics["scf_final_rms_mean"]]
    scf_final_rms_max_history = [initial_metrics["scf_final_rms_max"]]
    eval_steps = [0]
    ...
    logger.log(
        ...
        f"s1_total_weights=mse:{float(args.s1_total_mse_weight):.6g}/"
        f"mae:{float(args.s1_total_mae_weight):.6g} "
    ...
    t0 = time.perf_counter()
    progress_csv = Path(args.outdir) / "training_progress.csv"
    logger.log(f"[train_init] per-epoch progress: {progress_csv}")
    for step in range(1, int(args.steps) + 1):
        ...
        scf_converged_fraction_val = float(np.mean(scf_converged))
        scf_cycles_mean_val = float(np.mean(scf_cycles_mean))
        scf_cycles_max_val = float(np.max(scf_cycles_max))
        scf_final_rms_mean_val = float(np.mean(scf_final_rms_mean))
        scf_final_rms_max_val = float(np.max(scf_final_rms_max))
        best_updated = False
        ...
        scf_converged_fraction_history.append(scf_converged_fraction_val)
        scf_cycles_mean_history.append(scf_cycles_mean_val)
        scf_cycles_max_history.append(scf_cycles_max_val)
        scf_final_rms_mean_history.append(scf_final_rms_mean_val)
        scf_final_rms_max_history.append(scf_final_rms_max_val)
        current_lr = (
            float(lr_schedule(step - 1))
            if lr_schedule is not None
            else float(args.learning_rate)
        )
        _append_training_progress_row(
            progress_csv,
            step=step,
            loss=train_loss_val,
            s1_total_mae=train_s1_mae_val,
            scf_converged_fraction=scf_converged_fraction_val,
            scf_cycles_mean=scf_cycles_mean_val,
            scf_cycles_max=scf_cycles_max_val,
            scf_final_rms_mean=scf_final_rms_mean_val,
            scf_final_rms_max=scf_final_rms_max_val,
        ...
        if step == 1 or step % max(1, int(args.log_every)) == 0 or step == int(args.steps):
            ...
            logger.log(
                ...
                f"scf_cycles_mean={scf_cycles_mean_val:.3f} "
                f"scf_cycles_max={scf_cycles_max_val:.0f} "
                f"scf_final_rms_max={scf_final_rms_max_val:.8e} "
                f"lr={current_lr:.8e}"
            )
    ...
    scf_converged_fraction_history[-1] = final_metrics["scf_converged_fraction"]
    scf_cycles_mean_history[-1] = final_metrics["scf_cycles_mean"]
    scf_cycles_max_history[-1] = final_metrics["scf_cycles_max"]
    scf_final_rms_mean_history[-1] = final_metrics["scf_final_rms_mean"]
    scf_final_rms_max_history[-1] = final_metrics["scf_final_rms_max"]
    final_lr = (
        float(lr_schedule(max(0, int(args.steps) - 1)))
        if lr_schedule is not None
        else float(args.learning_rate)
    ...
    return {
        ...
        "scf_converged_fraction_history": scf_converged_fraction_history,
        "scf_cycles_mean_history": scf_cycles_mean_history,
        "scf_cycles_max_history": scf_cycles_max_history,
        "scf_final_rms_mean_history": scf_final_rms_mean_history,
        "scf_final_rms_max_history": scf_final_rms_max_history,
        "training_progress_csv": str(progress_csv),
...
def train_functional(
    ...
    *,
    ...
    training_data = build_s1_training_data(train_points)
    functional = _make_s1_functional(args)
    ...
    if args.init_checkpoint:
        ...
        logger.log(f"[train_init] loaded params from checkpoint: {init_checkpoint}")
    train_step = make_molecular_train_step(functional, training_config=gs_training)
    loss_and_grad_kernel = make_molecular_loss_and_grad(
        functional,
        training_config=gs_training,
    )
    ...
    def make_eval_fn(dataset):
        return lambda params: molecular_loss(  # noqa: E731
            params,
            functional,
            dataset,
            training_config=gs_training,
    ...
    for step in range(1, int(args.steps) + 1):
        ...
        if use_split_jit_train:
            ...
            train_metrics["param_update_norm"] = jnp.asarray(
                ...
                dtype=jnp.asarray(train_metrics["total_loss"]).dtype,
            )
...
def write_training_history_csv(path: Path, training: dict[str, Any]) -> None:
    ...
    scf_converged = np.asarray(
        training.get("scf_converged_fraction_history", np.full(steps.shape, np.nan)),
        dtype=np.float64,
    )
    scf_cycles_mean = np.asarray(
        training.get("scf_cycles_mean_history", np.full(steps.shape, np.nan)),
        dtype=np.float64,
    )
    scf_cycles_max = np.asarray(
        training.get("scf_cycles_max_history", np.full(steps.shape, np.nan)),
        dtype=np.float64,
    )
    scf_final_rms_mean = np.asarray(
        training.get("scf_final_rms_mean_history", np.full(steps.shape, np.nan)),
        dtype=np.float64,
    )
    scf_final_rms_max = np.asarray(
        training.get("scf_final_rms_max_history", np.full(steps.shape, np.nan)),
        dtype=np.float64,
    )
    eval_map = {
        ...
        for idx, step in enumerate(steps):
            ...
            writer.writerow(
                [
                    ...
                    float(scf_converged[idx]),
                    float(scf_cycles_mean[idx]),
                    float(scf_cycles_max[idx]),
                    float(scf_final_rms_mean[idx]),
                    float(scf_final_rms_max[idx]),
                    eval_loss_value,
                    eval_s1_value,
                ]
            )
...
def write_summary(
    ...
    *,
    ...
    eval_use_tda = (args.excited_state_solver == "tda") if args.eval_use_tda is None else bool(args.eval_use_tda)
    ...
    summary = {
        ...
        "training_progress_csv": training.get("training_progress_csv"),
    }
