...
_JAX_UKS_CACHE_VERSION = "spinpolarized-fci-dm-v4"
...
def _training_point_rows(
    ...
    for idx, point in enumerate(points):
        target = float(point.exact_energy_h)
...
def train_functional(points: list[ReferencePoint], *, args: argparse.Namespace, logger: RunLogger):
    ...
    per_point_rows = _training_point_rows(0, float(initial_loss), initial_metrics, points)
    logger.log(
        "[train] "
    ...
    for step in range(1, int(args.steps) + 1):
        ...
        loss = _metric_mean(metrics, "total_loss")
        ...
        rows.append(row)
        per_point_rows.extend(_training_point_rows(step, loss, metrics, points))
    ...
    if float(final_loss) < min_loss:
        ...
        best_params = state.params
    per_point_rows.extend(
        _training_point_rows(int(args.steps), float(final_loss), final_metrics, points)
    )
    return {
        ...
        "history": rows,
        "per_point_history": per_point_rows,
        "elapsed_s": time.perf_counter() - t0,
        "final_loss": float(final_loss),
        "final_energy_mae_h": _metric_mean(final_metrics, "e0_total_mae"),
        "min_loss": min_loss,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument("--network-architecture", choices=("graddft_residual",), default=DEFAULT_NETWORK_ARCHITECTURE)
...
def main(argv: list[str] | None = None) -> dict[str, Any]:
    ...
    write_rows(outdir / "training_per_point_history.csv", training["per_point_history"])
    ...
    summary = {
        ...
        "training_per_point_history_csv": str(outdir / "training_per_point_history.csv"),
        ...
        "figures": [
            {
                ...
                "data_files": [
                    ...
                    str(outdir / "training_per_point_history.csv"),
