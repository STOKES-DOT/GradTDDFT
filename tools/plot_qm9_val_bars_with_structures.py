#!/usr/bin/env python3
"""Plot QM9 validation S1/S2 bars with molecular structure insets."""
...
def _plot(rows: list[dict[str, str]], ref_by_system: dict[str, dict[str, str]], outdir: Path, xyzrender_src: Path) -> dict[str, Any]:
    ...
    fig.suptitle("QM9 validation excitation energies: EOM-CCSD vs trained Neural XC vs B3LYP", fontsize=12)
    png_path = outdir / "qm9_val_s1s2_bars_with_structures.png"
    pdf_path = outdir / "qm9_val_s1s2_bars_with_structures.pdf"
...
def main() -> None:
    ...
    plot_data_path = args.outdir / "qm9_val_s1s2_bar_plot_data.csv"
