...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Compare the legacy AO-projected HF channel against the DM21-style exact local "
            "HFX channel on the benzene molecular plane."
        )
    )
    ...
    p.add_argument("--outdir", default="outputs/benzene_hf_channel_mode_compare")
    return p.parse_args()


def _restricted_spin_density_blocks(ref) -> tuple[np.ndarray, np.ndarray]:
    rdm1 = np.asarray(ref.rdm1, dtype=np.float64)
    if rdm1.ndim == 2:
        dm = 0.5 * rdm1
        return dm, dm
    if rdm1.ndim != 3 or rdm1.shape[0] != 2:
        raise ValueError(f"Expected restricted rdm1 with shape (2, nao, nao), got {rdm1.shape}.")
    return rdm1[0], rdm1[1]


def _dm21_exact_hf_plane_profile(
    mol,
    ref,
    ...
    *,
    ...
    dm_a, dm_b = _restricted_spin_density_blocks(ref)
    values = np.zeros(coords_ang.shape[0], dtype=np.float64)

    for start in range(0, coords_ang.shape[0], int(chunk_size)):
        stop = min(start + int(chunk_size), coords_ang.shape[0])
        coords_bohr = coords_ang[start:stop] * ANG_TO_BOHR
        ao_chunk = mol.eval_gto("GTOval_cart", coords_bohr)
        nu_chunk = np.asarray(
            mol.intor("int1e_grids_cart", hermi=1, grids=coords_bohr),
            dtype=np.float64,
        )
        e_a = np.einsum("gp,pq->gq", ao_chunk, dm_a, optimize=True)
        e_b = np.einsum("gp,pq->gq", ao_chunk, dm_b, optimize=True)
        fxx_a = np.einsum("gbc,gc->gb", nu_chunk, e_a, optimize=True)
        fxx_b = np.einsum("gbc,gc->gb", nu_chunk, e_b, optimize=True)
        exx_a = -0.5 * np.einsum("gb,gb->g", e_a, fxx_a, optimize=True)
        exx_b = -0.5 * np.einsum("gb,gb->g", e_b, fxx_b, optimize=True)
        values[start:stop] = exx_a + exx_b
    return values
...
def _plot_plane_compare(
    *,
    ...
    dm21_exact: np.ndarray,
    outpath: Path,
) -> None:
    diff = legacy_projected - dm21_exact
    vmax = max(
        float(np.max(np.abs(legacy_projected))),
        float(np.max(np.abs(dm21_exact))),
        1e-12,
    )
    ...
    panels = [
        (legacy_projected, "Legacy AO-projected HF", "RdBu_r", vmax),
        (dm21_exact, "DM21 exact local HFX", "RdBu_r", vmax),
        (diff, "Legacy - DM21 exact", "coolwarm", dvmax),
    ]
    ...
    fig.suptitle("Benzene HF local channel on the molecular plane\nReference: restricted HF / 6-31G")
...
def _plot_axis_compare(
    *,
    ...
    axes[0].plot(x, legacy_projected, lw=2.0, color="#1f77b4", label="Legacy AO-projected")
    axes[0].plot(x, dm21_exact, lw=2.0, color="#d62728", label="DM21 exact local HFX")
    ...
    axes[1].set_ylabel("Legacy - DM21 (Eh)")
    axes[1].grid(alpha=0.2, linewidth=0.7)

    fig.suptitle("Benzene HF channel comparison on the symmetry axis\nReference: restricted HF / 6-31G, y = 0 A")
    fig.tight_layout()
    fig.savefig(outpath, bbox_inches="tight")
    plt.close(fig)
...
def main() -> None:
    ...
    legacy_grid = np.asarray(hf_data["projected_grid"], dtype=np.float64)
    dm21_grid = np.asarray(ref.hfx_local[0, :, 0] + ref.hfx_local[1, :, 0], dtype=np.float64)
    ...
    _, legacy_plane, _ = _hf_plane_profiles(
        mol,
        ref,
        plane_coords_ang,
        hf_data,
    ...
    dm21_plane = _dm21_exact_hf_plane_profile(
        mol,
        ref,
        plane_coords_ang,
        chunk_size=int(args.chunk_size),
    ...
    dm21_plane_2d = dm21_plane.reshape(len(y), len(x))
    ...
    _, legacy_axis, _ = _hf_plane_profiles(
        mol,
        ref,
        axis_coords_ang,
        hf_data,
    ...
    dm21_axis = _dm21_exact_hf_plane_profile(
        mol,
        ref,
        axis_coords_ang,
        chunk_size=int(args.chunk_size),
    ...
    plane_png = outdir / "benzene_hf_channel_mode_plane_compare.png"
    axis_png = outdir / "benzene_hf_channel_mode_axis_compare.png"
    _plot_plane_compare(
        x=x,
        y=y,
        legacy_projected=legacy_plane_2d,
        dm21_exact=dm21_plane_2d,
        outpath=plane_png,
    )
    _plot_axis_compare(
        x=axis_x,
        legacy_projected=legacy_axis,
        dm21_exact=dm21_axis,
        carbon_x=carbon_x,
    ...
    summary = {
        ...
        "legacy_scale_factor": float(hf_data["scale"]),
        "grid_legacy_integral_h": float(np.dot(grid_weights, legacy_grid)),
        "grid_dm21_integral_h": float(np.dot(grid_weights, dm21_grid)),
        "exact_exchange_h": float(hf_data["exact_energy_h"]),
        "grid_diff_mae_h": float(np.mean(np.abs(legacy_grid - dm21_grid))),
        "grid_diff_max_abs_h": float(np.max(np.abs(legacy_grid - dm21_grid))),
        "plane_diff_mae_h": float(np.mean(np.abs(legacy_plane_2d - dm21_plane_2d))),
        "plane_diff_max_abs_h": float(np.max(np.abs(legacy_plane_2d - dm21_plane_2d))),
        "axis_diff_mae_h": float(np.mean(np.abs(legacy_axis - dm21_axis))),
        "axis_diff_max_abs_h": float(np.max(np.abs(legacy_axis - dm21_axis))),
        "plane_png": str(plane_png),
        "axis_png": str(axis_png),
    }
    json_path = outdir / "benzene_hf_channel_mode_compare.json"
