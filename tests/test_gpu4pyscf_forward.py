...
def test_run_gpu4pyscf_rks_forward_includes_extra_fock_matrix(monkeypatch):
    import jax.numpy as jnp

    from td_graddft.nn_rsh.schema import SCFXCContributions
    from td_graddft.scf.gpu4pyscf import run_gpu4pyscf_rks_forward
    from td_graddft.scf.molecules import QuadratureGrid, RestrictedMolecule
    ...
    class ExtraFockFunctional:
        def bind_to_molecule_for_scf(self, params, molecule_in):
            del params, molecule_in
            return self

        def scf_contributions(self, molecule_in):
            del molecule_in
            return SCFXCContributions(
                v_rho=jnp.zeros((2,), dtype=jnp.float32),
                v_grad=jnp.zeros((2, 3), dtype=jnp.float32),
                xc_kind="LDA",
                full_hf_fraction=jnp.asarray(0.0, dtype=jnp.float32),
                extra_fock_matrix=jnp.diag(jnp.asarray([7.0, 11.0], dtype=jnp.float32)),
            )
    ...
    result = run_gpu4pyscf_rks_forward(
        ...
        xc_spec="pbe",
        molecule_template=molecule_template,
        xc_functional=ExtraFockFunctional(),
        xc_params={},
    )

    assert np.allclose(fake_mf_holder["mf"].recorded_veff, np.diag([11.0, 11.0]))
    assert np.allclose(result.fock_matrix, np.diag([11.0, 11.0]))
