...
def test_build_rks_integral_inputs_gpu_full_uses_gpu4pyscf_eri(monkeypatch):
    ...
    def fake_gpu_eri(*, packed, atom, basis, unit, charge, spin, cart, verbose, mol_kwargs):
        del atom, basis, unit, charge, spin, cart, verbose, mol_kwargs
        assert packed is True
        calls["eri"] += 1
        return fake_pair

    monkeypatch.setattr(inputs_mod, "_gpu4pyscf_eri", fake_gpu_eri)
...
def test_build_uks_integral_inputs_gpu_full_skips_jax_backbone(monkeypatch):
    ...
    def fake_gpu_tensor(*, packed, atom, basis, unit, charge, spin, cart, verbose, mol_kwargs):
        del atom, basis, unit, charge, spin, cart, verbose, mol_kwargs
    ...
    monkeypatch.setattr(inputs_mod, "_gpu4pyscf_eri", fake_gpu_tensor)
