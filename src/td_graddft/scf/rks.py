...
from .direct_jk import (
    _DIRECT_PACKED_JK_MAX_NAO,
    ...
    def _direct_builder() -> JKBuilder:
        ...
        if cfg.direct_jk_engine == "cuda" and cuda_ffi_available():
            ...
            if eri_pair_matrix is not None:
                ...
                def _direct_cuda_supplied_pair_jk(
                    ...
                    result_j, result_k = eval_cuda_supplied_pair(density)
                    if with_k:
            ...
            def _direct_cuda_jk(
                ...
                if use_incremental and density_last is not None:
                    ...
                    delta_j, delta_k = eval_cuda_direct(delta)
                    result_j = jnp.asarray(j_last) + delta_j
                    result_k = jnp.asarray(k_last) + delta_k
                else:
                    result_j, result_k = eval_cuda_direct(density)
                if with_k:
                    return result_j, result_k
                return result_j, jnp.zeros_like(density)
        ...
        if threshold <= 0.0 and int(direct_basis.nao) <= _DIRECT_PACKED_JK_MAX_NAO:
            ...
            def _direct_packed_jk(
                ...
                if cfg.direct_scf_incremental and density_last is not None:
                    ...
                    delta = jnp.asarray(density) - jnp.asarray(density_last)
                    delta_j, delta_k = build_jk_from_eri_pair_matrix(pair_arr, delta)
                    result_j = jnp.asarray(j_last) + delta_j
                    result_k = jnp.asarray(k_last) + delta_k
                else:
                    result_j, result_k = build_jk_from_eri_pair_matrix(pair_arr, density)
                if with_k:
                    return result_j, result_k
                return result_j, jnp.zeros_like(density)
        ...
        def _direct_jk(
            ...
            if cfg.direct_scf_incremental and density_last is not None:
                ...
                result = build_direct_jk_from_basis(
                    direct_basis,
                    density,
                    screening_threshold=threshold,
                    shell_pair_schwarz_bounds=bounds,
...
def _run_scf_iterations_lax(
    *,
    ...
    def cond_fn(carry):
        cycle, converged, *_ = carry
        return jnp.logical_and(cycle < cfg.max_cycle, jnp.logical_not(converged))
...
def _run_scf_iterations_lax_traceable(
    *,
    ...
    """LAX SCF loop returning array-valued metadata for geometry autodiff."""
...
def run_rks_from_integrals(
    *,
    ...
    enuc = _host_device_scalar(float(np.asarray(jax.device_get(nuclear_repulsion))), h.dtype)
    nao = int(s.shape[0])

    if _use_host_initial_orbitals(cfg):
    ...
    skip_first_fock_damping = init_mo_coeff is not None
    alpha_scalar = float(hybrid_coeff(cfg.xc_spec))
    alpha = _host_device_scalar(alpha_scalar, h.dtype)
    ...
    if _defer_initial_fock_build(cfg):
        fock = h
...
def run_rks_from_integrals_traceable(
    *,
    ...
    scf_iteration_runner = _run_scf_iterations_lax_traceable
    ...
    if cfg.level_shift != 0.0:
        (
            ...
            converged,
            lambda _: _run_extra_final_cycle(
                density=density,
                mo_coeff=mo_coeff,
                mo_occ=mo_occ_fixed,
                mo_energy=mo_energy,
