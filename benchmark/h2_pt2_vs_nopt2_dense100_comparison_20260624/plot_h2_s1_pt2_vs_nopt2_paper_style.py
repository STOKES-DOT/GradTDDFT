...
RUN_DIR = ROOT / "benchmark/h2_pt2_vs_nopt2_dense100_comparison_20260624"
SOURCE_CSV = RUN_DIR / "h2_pt2_vs_nopt2_dense100_comparison.csv"
PAPER_FIG_DIR = ROOT / "paper/tdgraddft-paper/figures"
...
CASES = [
    {
        "key": "pt2",
        "pred_col": "pt2_pred_s1_total_h",
        "abs_err_col": "pt2_s1_total_abs_err_ev",
        "gap_col": "pt2_pred_s1_gap_h",
    ...
    {
        "key": "no_pt2",
        "pred_col": "nopt2_pred_s1_total_h",
        "abs_err_col": "nopt2_s1_total_abs_err_ev",
        "gap_col": "nopt2_pred_s1_gap_h",
...
def make_rows(source_rows: list[dict[str, str]], case: dict[str, str]) -> list[dict[str, float]]:
    rows: list[dict[str, float]] = []
    for index, row in enumerate(source_rows, start=1):
        reference_energy = float(row["exact_s1_total_h"])
        predicted_energy = float(row[case["pred_col"]])
        reference_gap = float(row["exact_s1_gap_h"])
        predicted_gap = float(row[case["gap_col"]])
        rows.append(
            {
                "dense_index": float(row.get("dense_index", index)),
                "r_angstrom": float(row["r_angstrom"]),
                "reference_energy_h": reference_energy,
                "predicted_energy_h": predicted_energy,
                "signed_error_mev": (predicted_energy - reference_energy) * EV_PER_H * 1000.0,
                "abs_error_ev": float(row[case["abs_err_col"]]),
                "reference_gap_h": reference_gap,
                "predicted_gap_h": predicted_gap,
            }
        )
    return rows
...
def compute_metrics(rows: list[dict[str, float]], case: dict[str, str]) -> dict[str, object]:
    ...
    return {
        ...
        "dense_max_abs_error_ev_from_csv": abs_errors[max_index] / 1000.0,
...
def draw_case(
    ...
    *,
    ...
    if max(abs_errors) > 5.0 * KCAL_PER_MOL_TO_MEV:
        ax_bottom.text(
            0.055,
            0.88,
...
def save_figure(fig: plt.Figure, out_stem: str, *, include_paper: bool) -> dict[str, str]:
    output_dirs = [RUN_DIR]
    if include_paper:
        output_dirs.append(PAPER_FIG_DIR)
    ...
    for directory in output_dirs:
        ...
        label = "paper" if directory == PAPER_FIG_DIR else "run"
...
def make_single(case: dict[str, str], rows: list[dict[str, float]], metrics: dict[str, object]) -> None:
...
def make_combined(case_data: list[tuple[dict[str, str], list[dict[str, float]], dict[str, object]]]) -> dict[str, object]:
    ...
    outputs = save_figure(fig, COMBINED_OUT_STEM, include_paper=True)
    plt.close(fig)
    combined_metrics = {
        "system": "H2",
        "state": "S1",
